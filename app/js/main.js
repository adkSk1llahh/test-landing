...
for (const option of document.querySelectorAll(".navbar-option")) {
    option.addEventListener('click', function () {
        if (!this.classList.contains('selected')) {
            this.parentNode.querySelector('.navbar-option.selected').classList.remove('selected');
            this.classList.add('selected');
            this.closest('.navbar-select').querySelector('.navbar-select__trigger span').textContent = this.textContent;
        }
    })
}
